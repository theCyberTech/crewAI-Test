...
This is a complete, copy-pasteable example. It needs an LLM API key in the environment (for example `OPENAI_API_KEY`). `role`, `goal`, and `backstory` are required.
...
`kickoff()` returns a `LiteAgentOutput`. Use `result.raw` for the text answer. If you passed `response_format`, use `result.pydantic` for the parsed model.

To give the agent tools, pass them at construction time:
