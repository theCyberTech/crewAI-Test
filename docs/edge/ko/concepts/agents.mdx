...
다음은 복사해서 바로 실행할 수 있는 완전한 예제입니다. 환경에 LLM API 키(예: `OPENAI_API_KEY`)가 필요합니다. `role`, `goal`, `backstory`는 필수입니다.
...
`kickoff()`는 `LiteAgentOutput`을 반환합니다. 텍스트 답변은 `result.raw`를 사용하세요. `response_format`을 전달했다면 파싱된 모델은 `result.pydantic`입니다.

에이전트에 도구를 주려면 생성 시점에 전달하세요:
