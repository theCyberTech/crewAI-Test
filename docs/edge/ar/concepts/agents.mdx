...
هذا مثال كامل يمكن نسخه وتشغيله. يحتاج إلى مفتاح API لنموذج لغوي في البيئة (مثل `OPENAI_API_KEY`). `role` و`goal` و`backstory` حقول مطلوبة.
...
يعيد `kickoff()` كائن `LiteAgentOutput`. استخدم `result.raw` للإجابة النصية. إذا مرّرت `response_format`، استخدم `result.pydantic` للنموذج المُحلَّل.

لمنح الوكيل أدوات، مرّرها عند الإنشاء:
